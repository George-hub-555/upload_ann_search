 ...
 *
 * Run:
 *   ./bench_glove_custom                         (uses default dataset)
 *   ./bench_glove_custom -dataset /path/to/file  (custom path)
 *   ./bench_glove_custom -k 20 -maxturn 200000 -reportfreq 1000
 *
 * All flags are optional:
 *   -dataset     <path>  GloVe dataset path (default: dataset/glove.twitter.27B.25d.txt)
...
/* =============================================================
 *  Pick the best m for PQ: divisor of d closest to 8, max 64
 * ============================================================= */
...
int main(int argc, char** argv) {
    ...
    /* ---- Parse flags ---- */
    std::string dataset_path = arg_get_str(
        argc, argv, "-dataset", "dataset/glove.twitter.27B.25d.txt");
    ...
    if (arg_has(argc, argv, "-h") || arg_has(argc, argv, "--help")) {
        ...
        printf("  -dataset     <path>  GloVe dataset path  (default: dataset/glove.twitter.27B.25d.txt)\n");
    ...
    printf("  dataset=%s  k=%d  maxturn=%zu  reportfreq=%d\n",
           dataset_path.c_str(), k, maxturn, reportfreq);
    printf("============================================================\n");
    ...
    /* =============================================================
     *  run_search lambda — with progress CSV if reportfreq > 0
     * ============================================================= */
    auto run_search = [&](const char* label, const char* params_tag,
        ...
        /* Build a safe filename for the progress CSV */
     ...
     * ============================================================= */
    {
        ...
        for (int nprobe : {1, 2, 4, 8, 16, 32, 64}) {
            ...
            snprintf(params, sizeof(params), "nlist=%d nprobe=%d",
                     nlist, nprobe);
            run_search("IVFFlat", params, ivf_flat, nq, recall, qps);
     ...
     * ============================================================= */
    {
        ...
        faiss::IndexIVFPQ ivf_pq(&qz, d, nlist, m, 8);
        ivf_pq.train(nt, xt);
        ivf_pq.add(nb, xb);
        double t_build = elapsed() - t0;
     ...
     * ============================================================= */
    {
        ...
        for (int efSearch : {16, 32, 64, 128, 256, 512}) {
            ...
            snprintf(params, sizeof(params), "M=%d efSearch=%d",
                     M, efSearch);
            run_search("HNSW", params, hnsw, nq, recall, qps);
