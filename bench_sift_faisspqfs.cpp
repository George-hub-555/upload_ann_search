/*
 * bench_sift_faisspqfs.cpp - FAISS SIFT1M QPS / Recall benchmark for ARM
 *
...
void print_help(const char* program) {
    ...
    std::printf(
            "  -k_reorder   <int>   exact rerank factor; 0 disables (default: 0)\n");
    std::printf(
            "  -bbs         <int>   FastScan block size, multiple of 32 (default: 32)\n");
    std::printf(
            "  -hnsw_m      <int>   HNSW links per node (default: 32)\n");
    std::printf(
            "  -output      <path>  result CSV (default: "
            "bench_sift_faisspqfs_results.csv)\n");
}
...
int benchmark_main(int argc, char** argv) {
    ...
    const std::string output_path =
            arg_get_str(argc, argv, "-output", "bench_sift_faisspqfs_results.csv");
    ...
    std::printf("============================================================\n");
    std::printf("FAISS SIFT IVFPQ FastScan Comparison Benchmark\n");
    std::printf(
    ...
    if (base.dimension % static_cast<size_t>(pq_m) != 0) {
        throw std::runtime_error(
                "-m must divide the SIFT dimension (" +
                std::to_string(base.dimension) + ")");
    }
    if (k_reorder > 0) {
        const size_t reorder = static_cast<size_t>(k_reorder);
        if (static_cast<size_t>(k) > base.count / reorder) {
            throw std::runtime_error(
    ...
    if (selected_indexes.count("ivfflat")) {
        ...
        faiss::IndexIVFFlat index(
                &quantizer, dimension, static_cast<size_t>(nlist));
        index.train(
    ...
    if (selected_indexes.count("ivfpqfs")) {
        std::printf(
                "[%.3f s] Training/building IVFPQFastScan(nlist=%d, "
                "m=%d, nbits=4, bbs=%d) ...\n",
                elapsed_seconds() - program_start,
                nlist,
                pq_m,
